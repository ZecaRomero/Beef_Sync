...
const dbConfig = process.env.DATABASE_URL
  ...
  : (() => {
      ...
      // Em desenvolvimento: usa .env; fallback apenas se variável não carregar (ex: scripts)
      const resolvedPassword = password || (process.env.NODE_ENV === 'development' ? 'jcromero85' : '');
      return {
...
if (process.env.NODE_ENV !== 'production') {
  ...
  console.log('DB Connection Config:', safeConfig);
}
  ...
  try {
    ...
    await client.query(`
      ...
      BEGIN
        ...
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'animais' AND column_name = 'genetica_2') THEN
          ALTER TABLE animais ADD COLUMN genetica_2 VARCHAR(50);
        END IF;
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'animais' AND column_name = 'decile_2') THEN
          ALTER TABLE animais ADD COLUMN decile_2 VARCHAR(50);
        END IF;
      END $$;
    `)
