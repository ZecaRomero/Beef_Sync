...
const parsearTextoImport = (texto) => {
  ...
  // Tab, vÃ­rgula (se nÃ£o houver nÃºmeros com vÃ­rgula decimal) ou espaÃ§o
  ...
  const skipHeader = header.includes('SÃâ€°RIE') || header.includes('SERIE') || header.includes('SERII') || header.includes('RG') || header.includes('RGN')
  ...
  // Formato SÃ©rie | RG | MGTe | TOP (4 colunas)
  const formatoMGTeTOP = !formatoCompleto9Cols && header.includes('MGTE') && header.includes('TOP')
  // Formato completo 6 colunas: SÃâ€°RIE | RG | iABCZg | DECA | IQG | Pt IQG
  ...
  const col7Situacao = headerCols[6] && (String(headerCols[6]).toUpperCase().includes('SITUAÃâ€¡ÃÆ’O') || String(headerCols[6]).toUpperCase().includes('SITUACAO') || String(headerCols[6]).toUpperCase().includes('STATUS'))
  const formatoCompleto7Cols = formatoCompleto6Cols && col7Situacao
  // Sem header: 6 cols com col2-5 numÃ©ricos = iABCZ, DECA, IQG, Pt IQG
  ...
  // Formato: SÃ©rie, RG, IQG, Pt IQG (4 cols)
  const formatoIQGPeloHeader = headerCols.length >= 4 && headerCols.length <= 6 &&
    ((header.includes('IQG') || header.includes('IQGG')) && (header.includes('PT') || header.includes('PL'))) &&
    !header.includes('IABCZ') && !header.includes('DECA')
  // Sem header: detectar por dados (4 cols, col3 e col4 numÃ©ricos = IQG) ou 3 cols com serie+rg em col0
  ...
  for (let i = start; i < linhas.length; i++) {
    ...
    if (cols.length >= 2) {
      ...
      // Se col0 parece "SÃâ€°RIE RG" (ex: "CJCJ 17267") e col1 Ã© numÃ©rico (iABCZ/IQG), extrair da col0
      ...
      if (formatoStatus && cols.length >= 2 + offsetCols) {
        ...
        // SÃ©rie, RG, iABCZg, DECA, IQG, Pt IQG (6 cols) + opcional SituaÃ§Ã£o ABCZ (7Âª col, pode ter espaÃ§o ex: "POSSUI RGN")
...
export default function ImportGeneticaModal({ isOpen, onClose, onSuccess }) {
  ...
  const formatarResultadoImport = (r) => {
    ...
    let msg = `ImportaÃ§Ã£o concluÃ­da! ${n} animal(is) atualizado(s).`
    if (criados > 0) msg += ` ${criados} criado(s).`
    if (limpos > 0) msg += ` ${limpos} limpo(s) (fora da planilha).`
    if (ignorados > 0) {
      msg += ` Os demais (${ignorados}) foram ignorados (nÃ£o estÃ£o no cadastro ou estÃ£o inativos).`
    }
    if (n === 0 && criados === 0 && (r.naoEncontrados?.length || 0) > 0) {
      const amostra = (r.naoEncontrados || []).slice(0, 5).map(x => `${x.serie || '?'} ${x.rg || '?'}`.trim()).join(', ')
      msg += `\n\nExemplos nÃ£o encontrados: ${amostra}. Marque "Criar animais nÃ£o cadastrados" para cadastrar automaticamente.`
    }
    if ((r.erros?.length || 0) > 0) {
      const amostraErro = (r.erros || []).slice(0, 3).map(x => `${x.serie || '?'} ${x.rg || '?'} (${x.erro})`).join(', ')
      msg += `\n\nâÅ¡ ï¸� ${r.erros.length} erro(s) ao importar. Ex: ${amostraErro}`
    }
    return msg
  }
  ...
  const handleImportar = async () => {
    ...
    try {
      console.log('[Import GenÃ©tica] Iniciando...')
      if (importMode === 'texto') {
        const dados = parsearTextoImport(importTexto)
        if (dados.length === 0) {
          setResultadoImport({ erro: 'Nenhum dado vÃ¡lido. Use tab ou espaÃ§o entre colunas. Ex: SÃ©rie RG iABCZ Deca ou SÃ©rie RG IQG Pt' })
          return
        }
        ...
        console.log('[Import GenÃ©tica] Resposta:', res.status, json)
        if (res.ok) {
          ...
          setResultadoImport({ erro: json.error || json.details || 'Erro na importaÃ§Ã£o' })
          alert(`â�Å’ Erro: ${json.error || json.details || 'Erro na importaÃ§Ã£o'}`)
        }
      } else {
        if (!importFile) {
        ...
        const json = await res.json()
        console.log('[Import GenÃ©tica] Resposta Excel:', res.status, json)
        if (res.ok) {
          setResultadoImport(json)
          setImportFile(null)
          onSuccess?.()
      ...
      console.error('[Import GenÃ©tica] Erro:', err)
      setResultadoImport({ erro: err.message || 'Erro ao importar' })
      alert(`â�Å’ Erro: ${err.message || 'Verifique a conexÃ£o e tente novamente.'}`)
    } finally {
      setImportando(false)
    }
  }
  ...
  const handleLimparTodas = async () => {
    ...
    if (!confirm('ZERAR TODOS os dados genÃ©ticos (iABCZ, DECA, IQG, Pt IQG, SituaÃ§Ã£o ABCZ) de TODOS os animais?\n\nDepois importe o Excel com 6 ou 7 colunas. O 17098 e outros fora da planilha deixarÃ£o de aparecer no ranking.')) return
    setLimpando(true)
    setResultadoImport(null)
    try {
      ...
      if (res.ok) {
        ...
        onSuccess?.()
        alert(`${json.message}\n\nAgora importe o Excel (6 ou 7 colunas: SÃ©rie, RG, iABCZg, DECA, IQG, Pt IQG, SituaÃ§Ã£o ABCZ).`)
      } else {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Importar GenÃ©tica (iABCZ, DECA, IQG, Pt IQG)" size="lg">
      <div className="space-y-4">
        <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
          <p className="text-sm font-semibold text-amber-800 dark:text-amber-200 mb-1">ðÅ¸â€œâ€¹ Para reimportar do zero:</p>
          <p className="text-xs text-amber-700 dark:text-amber-300">1) Clique em &quot;Zerar tudo&quot; abaixo ââ€ â€™ 2) Importe o Excel com 6 ou 7 colunas (SÃ©rie, RG, iABCZg, DECA, IQG, Pt IQG, SituaÃ§Ã£o ABCZ)</p>
        </div>
        ...
        {importMode === 'texto' ? (
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Cole os dados (tab entre colunas). <strong>MGTe/TOP:</strong> SÃ©rie, RG, MGTe, TOP (4 cols ââ‚¬â€� nÃ£o altera iABCZ/DECA/IQG). <strong>Completo:</strong> SÃ©rie, RG, iABCZg, DECA, IQG, Pt IQG (6 cols) ou + SituaÃ§Ã£o ABCZ (7 cols). Ou sÃ³ IQG: SÃ©rie, RG, IQG, Pt IQG (4 cols).</p>
            <textarea
              value={importTexto}
              onChange={(e) => setImportTexto(e.target.value)}
              placeholder="SÃâ€°RIE	RG	iABCZg	DECA	IQG	Pt IQG	SituaÃ§Ã£o ABCZ&#10;CJCS	1	34,8	1	45,93	0,1	E&#10;CJCS	3	40,5	1	53,86	0,5	E"
          ...
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
              <strong>Tudo junto (9 colunas):</strong> <strong>SÃ©rie | RG | MGTe | TOP | iABCZ | DECA | IQG | Pt IQG | situaÃ§Ãµes_abcz</strong> ââ‚¬â€� importa tudo de uma vez.
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
              <strong>MGTe e TOP:</strong> 4 colunas <strong>SÃ©rie | RG | MGTe | TOP</strong>. <strong>Completo:</strong> 6 ou 7 colunas <strong>SÃ©rie | RG | iABCZg | DECA | IQG | Pt IQG | SituaÃ§Ã£o ABCZ</strong>.
            </p>
            <input
              type="file"
              accept=".xlsx,.xls,.csv"
            ...
            />
            {importFile && <p className="text-sm text-green-600 mt-1">âÅ“â€œ {importFile.name}</p>}
          </div>
        )}
        ...
        {resultadoImport?.success && (
          <div className="p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg text-sm text-green-700 dark:text-green-300">
            âÅ“â€¦ {resultadoImport.message}
            {resultadoImport.resultados?.naoEncontrados?.length > 0 && (
              <p className="mt-1">NÃ£o encontrados: {resultadoImport.resultados.naoEncontrados.length}</p>
            )}
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-sm text-gray-600 dark:text-gray-400">Criar animais nÃ£o cadastrados (cadastra automaticamente os que nÃ£o existem)</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
            ...
            className="rounded border-gray-300 dark:border-gray-600"
          />
          <span className="text-sm text-gray-600 dark:text-gray-400">Limpar iABCZ/DECA de animais que nÃ£o estÃ£o na planilha (mesma sÃ©rie) ââ‚¬â€� <em>nÃ£o se aplica a MGTe/TOP</em></span>
        </label>
        <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-gray-200 dark:border-gray-600">
          <button
            ...
            title="Zera iABCZ, DECA, IQG, Pt IQG de TODOS os animais. FaÃ§a isso antes de importar."
          >
