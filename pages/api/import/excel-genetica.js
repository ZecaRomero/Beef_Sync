...
/**
 * Extrai valor efetivo de cÃÆ’ÂÆ’Ãâ€šÂ©lula Excel (fÃÆ’ÂÆ’Ãâ€šÂ³rmulas retornam .result)
 */
...
/**
 * Normaliza valor numÃÆ’ÂÆ’Ãâ€šÂ©rico (iABCZ pode vir com vÃÆ’ÂÆ’Ãâ€šÂ­rgula: 47,71).
 * Trata objetos do Excel (fÃÆ’ÂÆ’Ãâ€šÂ³rmulas, richText) e evita "[object Object]".
 */
...
/**
 * Normaliza texto (SÃÆ’ÂÆ’Ãâ€šÂ©rie, RG, Deca). Evita retornar "[object Object]".
 */
...
/**
 * Normaliza RG: remove zeros ÃÆ’ÂÆ’Ãâ€šÂ  esquerda quando for numÃÆ’ÂÆ’Ãâ€šÂ©rico (ex: 017328 -> 17328)
...
/**
 * Importa SÃÆ’ÂÆ’Ãâ€šÂ©rie, RG, iABCZ, Deca de animais.
 * Formato esperado: SÃÆ’ÂÆ’Ãâ€šÂ©rie (A) | RG (B) | iABCZ (C) | Deca (D)
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      let msg = `ImportaÃÆ’ÂÆ’Ãâ€šÂ§ÃÆ’ÂÆ’Ãâ€šÂ£o concluÃÆ’ÂÆ’Ãâ€šÂ­da: ${resultados.animaisAtualizados} animais atualizados`;
    ...
    if (err) {
      console.error('Erro ao fazer parse do formulÃÆ’ÂÆ’Ãâ€šÂ¡rio:', err);
    ...
    if (!filepath) {
      return res.status(400).json({ error: 'Arquivo invÃÆ’ÂÆ’Ãâ€šÂ¡lido' });
    }
    ...
    if (cellA1.includes('SÃÆ’ÂÆ’Ã¢Ââ‚¬Â°RIE') || cellA1.includes('SERIE') || cellA1.includes('SERIE') || cellB1.includes('RG') || cellB1.includes('RGN')) {
      startRow = 2;
    }

    // Formato SÃÆ’ÂÆ’Ãâ€šÂ©rie|RGN|Status (3 colunas) - ex: SERIE, RGN, Status
    ...
    // Formato completo 9 colunas: SÃÆ’ÂÆ’Ãâ€šÂ©rie | RG | MGTe | TOP | iABCZ | DECA | IQG | Pt IQG | situaÃÆ’ÂÆ’Ãâ€šÂ§ÃÆ’ÂÆ’Ãâ€šÂµes_abcz
    ...
    // Formato SÃÆ’ÂÆ’Ãâ€šÂ©rie | RG | MGTe | TOP (4 colunas) - ex: planilha de mÃÆ’ÂÆ’Ãâ€šÂ©rito genÃÆ’ÂÆ’Ãâ€šÂ©tico (ANCP, etc)
    const formatoMGTeTOP = !formatoCompleto9Cols && /MGTE|MGT\b/i.test(String(cellC1 || '').trim()) && /TOP/i.test(String(cellD1 || '').trim());
    // Formato completo 6 colunas: SÃÆ’ÂÆ’Ã¢Ââ‚¬Â°RIE | RG | iABCZg | DECA | IQG | Pt IQG
    ...
    // Formato 7 colunas: SÃÆ’ÂÆ’Ãâ€šÂ©rie | RG | iABCZg | DECA | IQG | Pt IQG | SituaÃÆ’ÂÆ’Ãâ€šÂ§ÃÆ’ÂÆ’Ãâ€šÂ£o ABCZ
    const formatoCompleto7Cols = formatoCompleto6Cols && (cellG1.includes('SITUAÃÆ’ÂÆ’Ã¢Ââ‚¬Â¡ÃÆ’ÂÆ’Ãâ€ Ââ€™O') || cellG1.includes('SITUACAO') || cellG1.includes('STATUS'));
    ...
    // Formato SÃÆ’ÂÆ’Ãâ€šÂ©rie|RG|IQG/IQGg|Pt (4 colunas) - col 3 e 4 vÃÆ’ÂÆ’Ãâ€šÂ£o para IQG e Pt IQG (nÃÆ’ÂÆ’Ãâ€šÂ£o iABCZ/DECA)
    ...
    console.error('ÃÆ’Â¢Ãâ€šÂ�Ãâ€¦Ââ€™ Erro ao importar genÃÆ’ÂÆ’Ãâ€šÂ©tica:', error);
    ...
    return res.status(500).json({
      error: 'Erro ao processar importaÃÆ’ÂÆ’Ãâ€šÂ§ÃÆ’ÂÆ’Ãâ€šÂ£o',
      details,
    });
  }
}

/** Tenta INSERT com iqg/pt_iqg; se coluna nÃÆ’ÂÆ’Ãâ€šÂ£o existir, usa genetica_2/decile_2 */
  ...
  try {
    return await query(
      `INSERT INTO animais (nome, serie, rg, tatuagem, sexo, raca, situacao, abczg, deca, situacao_abcz, iqg, pt_iqg)
       VALUES ($1, $2, $3, $4, 'NÃÆ’ÂÆ’Ãâ€šÂ£o informado', 'NÃÆ’ÂÆ’Ãâ€šÂ£o informada', 'Ativo', $5, $6, $7, NULLIF(TRIM(REPLACE($8::text, ',', '.')), '')::numeric, NULLIF(TRIM(REPLACE($9::text, ',', '.')), '')::numeric)
    ...
    if (/column.*does not exist/i.test(e?.message || '')) {
      return await query(
        `INSERT INTO animais (nome, serie, rg, tatuagem, sexo, raca, situacao, abczg, deca, situacao_abcz, genetica_2, decile_2)
         VALUES ($1, $2, $3, $4, 'NÃÆ’ÂÆ’Ãâ€šÂ£o informado', 'NÃÆ’ÂÆ’Ãâ€šÂ£o informada', 'Ativo', $5, $6, $7, NULLIF(TRIM(REPLACE($8::text, ',', '.')), '')::numeric, $9::text)
         ON CONFLICT (serie, rg) DO UPDATE SET
...
async function processarLinhas(rows, criarNaoEncontrados = false, limparForaDaLista = false) {
  ...
  // Normalizar e filtrar linhas (RG sem zeros ÃÆ’ÂÆ’Ãâ€šÂ  esquerda para matching)
  const dados = [];
  for (let i = 0; i < rows.length; i++) {
    ...
    // Se RG contÃ©m hÃÆ’ÂÆ’Ãâ€šÂ­fen, pode estar no formato "CJCJ-16310" (sÃÆ’ÂÆ’Ãâ€šÂ©rie-rg junto)
    if (rg && rg.includes('-')) {
      const partes = rg.split('-');
      if (partes.length === 2) {
        // Se sÃÆ’ÂÆ’Ãâ€šÂ©rie estÃÆ’ÂÆ’Ãâ€šÂ¡ vazia, usar a primeira parte como sÃÆ’ÂÆ’Ãâ€šÂ©rie
        if (!serie) {
          serie = partes[0].trim();
        }
    ...
    dados.push({
      ...
      situacaoAbcz: apenasMgteTop ? null : (normalizarTexto(r.situacaoAbcz ?? r.situacao_abcz ?? r['SituaÃÆ’ÂÆ’Ãâ€šÂ§ÃÆ’ÂÆ’Ãâ€šÂ£o ABCZ'] ?? r.Status) || null),
      genetica2: apenasMgteTop ? null : safeNum(r.iqg ?? r.genetica_2 ?? r.genetica2 ?? r['IQG'] ?? r['GenÃÆ’ÂÆ’Ãâ€šÂ©tica 2']),
      decile2: decile2Val,
  ...
  // Buscar animais em lotes (evita query muito grande) - RG normalizado no banco (sem zeros ÃÆ’ÂÆ’Ãâ€šÂ  esquerda)
  ...
  for (let b = 0; b < dados.length; b += BATCH_SELECT) {
    ...
    // 1) Busca por serie + rg (RG normalizado: sem zeros ÃÆ’ÂÆ’Ãâ€šÂ  esquerda no banco)
    ...
    // 2) Fallback: buscar por tatuagem (serie-rg, serie+rg, serie+espaÃÆ’ÂÆ’Ãâ€šÂ§o+rg) para os que nÃÆ’ÂÆ’Ãâ€šÂ£o foram encontrados
  ...
  for (let b = 0; b < dados.length; b += BATCH) {
    ...
    if (updatesMgteTop.length > 0) {
      try {
        ...
        if (/column.*mgte|column.*top|does not exist/i.test(e?.message || '')) {
          console.warn('[excel-genetica] Colunas mgte/top nÃÆ’ÂÆ’Ãâ€šÂ£o existem, ignorando import MGTe/TOP');
        } else {
    ...
    const runUpdate = async (withMgteTop = temMgteTop) => {
      if (temColGenetica2) {
        try {
          ...
          } else if (/column.*iqg|column.*genetica|coluna.*nÃÆ’ÂÆ’Ãâ€šÂ£o existe/i.test(e?.message || '')) {
    ...
    try {
      ...
      if (/column.*does not exist|coluna.*nÃÆ’ÂÆ’Ãâ€šÂ£o existe/i.test(colErr?.message || '')) {
        temColGenetica2 = false;
        try {
  ...
  // Limpar iABCZ/DECA de animais da mesma sÃÆ’ÂÆ’Ãâ€šÂ©rie que NÃÆ’ÂÆ’Ãâ€ Ââ€™O estÃÆ’ÂÆ’Ãâ€šÂ£o na planilha
