...
export default function AutoCleanup() {
  ...
  const performCleanup = async () => {
    try {
      setStep(1)
      setStatus('≈∏ßπ Limpando localStorage...')
      ...
      setStep(2)
      setStatus('≈∏‚Äù‚Äû Verificando PostgreSQL...')
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.length === 0) {
            setStatus('‚≈ì‚Ä¶ PostgreSQL j√° est√° limpo')
          } else {
            setStatus(`‚≈°†Ô∏è PostgreSQL ainda tem ${data.length} animais`)
          }
        }
      } catch (error) {
        setStatus('‚ù≈í Erro ao verificar PostgreSQL')
      }
      
      await new Promise(resolve => setTimeout(resolve, 1000))
      
      setStep(3)
      setStatus('‚≈ì‚Ä¶ Limpeza conclu√≠da!')
      setCompleted(true)
      ...
      setStatus('‚ù≈í Erro na limpeza: ' + error.message)
    }
  }
          ...
          {completed && (
            <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
              <p className="text-green-700 dark:text-green-300 text-sm">
                ‚≈ì‚Ä¶ Limpeza conclu√≠da com sucesso!<br/>
