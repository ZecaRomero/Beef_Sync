...
export default function LimparDadosBanco() {
  ...
  const limparTodosDados = async () => {
    if (!confirm('‚≈°†Ô∏è ATEN√‚Ä°√∆íO: Esta a√ß√£o ir√° remover dados mock do banco de dados PostgreSQL. Tem certeza?')) {
      return
    }

    if (!confirm('≈∏≈°® CONFIRMA√‚Ä°√∆íO FINAL: Os dados ser√£o removidos permanentemente do banco. Continuar?')) {
      return
    }
    ...
    try {
      ...
      if (sucessos > 0) {
        setConcluido(true)
        setTimeout(() => {
          alert(`‚≈ì‚Ä¶ Limpeza conclu√≠da! ${sucessos} tipos de dados removidos, ${erros} erros.`)
          verificarDadosMock() // Recarregar dados
        }, 2000)
      } else {
        alert('‚‚ÄûπÔ∏è Nenhum dado mock foi encontrado para remover.')
      }

    } catch (error) {
      console.error('Erro durante a limpeza:', error)
      alert('‚ù≈í Erro durante a limpeza: ' + error.message)
    } finally {
      setLimpando(false)
    }
  }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                          ...
                          <div className="bg-gray-50 dark:bg-gray-700 rounded p-3 mt-2">
                            ...
                            {info.exemplos.map((exemplo, index) => (
                              <div key={index} className="text-xs text-gray-800 dark:text-gray-200 mb-1">
                                ‚‚Ç¨¢ {exemplo.receptora_nome || exemplo.doadora_nome || exemplo.nome || exemplo.rg || 'Item mock'}
                              </div>
                            ))}
                          </div>
                        )}
          ...
          <div className="mt-8 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <h3 className="font-medium text-gray-900 dark:text-white mb-2">
              ≈∏‚Äî‚ÄûÔ∏è Sobre esta Limpeza:
            </h3>
            <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
              <li>‚‚Ç¨¢ Remove dados fict√≠cios do banco PostgreSQL</li>
              <li>‚‚Ç¨¢ Procura por padr√µes como "Vaca 001", "Vaca 002", etc.</li>
              <li>‚‚Ç¨¢ Opera√ß√£o irrevers√≠vel - fa√ßa backup se necess√°rio</li>
              <li>‚‚Ç¨¢ Acesse: <code>localhost:3020/limpar-dados-banco</code></li>
            </ul>
          </div>
        </div>
      </div>
