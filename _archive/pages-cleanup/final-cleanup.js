...
export default function FinalCleanup() {
  ...
  const performFinalCleanup = async () => {
    try {
      setStep(1)
      setStatus('≈∏‚Äùç Verificando PostgreSQL...')
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.length === 0) {
            setStatus('‚≈ì‚Ä¶ PostgreSQL est√° limpo (0 animais)')
          } else {
            setStatus(`‚≈°†Ô∏è PostgreSQL tem ${data.length} animais`)
          }
        }
      } catch (error) {
      ...
      setStep(3)
      setStatus('‚≈ì‚Ä¶ Limpeza final conclu√≠da!')
      setCompleted(true)
          ...
          {completed && (
            <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
              <p className="text-green-700 dark:text-green-300 text-sm">
                ‚≈ì‚Ä¶ Sistema completamente limpo!<br/>
