 ...
 * - IA com IDEAL GUADALUPE ‚‚Ä†‚Äô Prenha (gesta√ß√£o atual)
 * - IA com REM 11627, JAMANTA, LANDROVES DA XARAES ou qualquer outro ‚‚Ä†‚Äô Vazia
 *
...
async function corrigir() {
  console.log('≈∏‚Äùß Corrigindo IAs da CJCJ 15959 (MOSCA SANT ANNA)\n')
  ...
  try {
    ...
    if (!temResultadoDg) {
      ...
      console.log('   ‚≈ì‚Ä¶ Coluna resultado_dg adicionada.')
      temResultadoDg = true
    }
    if (!temStatusGestacao) {
      ...
      console.log('   ‚≈ì‚Ä¶ Coluna status_gestacao adicionada.')
    ...
    if (animal.rows.length === 0) {
      console.log('‚ù≈í Animal CJCJ 15959 n√£o encontrado.')
      process.exit(1)
    }
    ...
    if (ins.rows.length === 0) {
      console.log('   ‚≈°†Ô∏è Nenhuma insemina√ß√£o encontrada.')
      process.exit(0)
    }
    ...
    for (const row of ins.rows) {
      ...
      if (ehIdealGuadalupe(touro)) {
        ...
        console.log(`   ‚≈ì‚Ä¶ IA ${row.id} (${touro}) ‚‚Ä†‚Äô Prenha`)
        atualizados++
      } else {
        await query(
        ...
        console.log(`   ‚≈ì‚Ä¶ IA ${row.id} (${touro}) ‚‚Ä†‚Äô Vazia`)
        atualizados++
      }
    }

    if (atualizados === 0) {
      console.log('   ‚‚ÄûπÔ∏è Nenhuma IA com IDEAL GUADALUPE ou touros REM/JAMANTA/LANDROVES encontrada.')
      console.log('   Verifique os nomes dos touros no banco.')
    } else {
      console.log(`\n   ≈∏‚Äú‚Äπ ${atualizados} registro(s) atualizado(s).`)
    }

    console.log('\n   ≈∏‚Äô° Acesse /consulta-animal/293 ou busque CJCJ 15959 para ver o touro correto (IDEAL GUADALUPE) e a previs√£o de parto.')
  } catch (err) {
