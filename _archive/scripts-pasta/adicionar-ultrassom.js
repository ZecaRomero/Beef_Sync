...
async function adicionarUltraSom() {
  ...
  try {
    ...
    console.log(`\nðÅ¸Å¡â‚¬ Iniciando processamento...`);
    console.log(`ðÅ¸â€œÅ  Total de animais: ${animaisIds.length}`);
    console.log(`ðÅ¸â€™° Custo total: R$ ${custoTotal.toFixed(2)}`);
    console.log(`ðÅ¸â€™µ Custo individual: R$ ${custoIndividual.toFixed(2)}`);
    console.log(`ðÅ¸â€œâ€¦ Data do ultrassom: ${dataUltraSom}\n`);
    ...
    for (const animalId of animaisIds) {
      try {
        ...
        if (animalCheck.rows.length === 0) {
          console.log(`âÅ¡ ï¸  Animal ID ${animalId} nÃ£o encontrado`);
        ...
        console.log(`âÅ“â€¦ Processando animal ${animal.serie || ''}/${animal.rg || ''} (ID: ${animalId})`);
        ...
        console.error(`âÅ’ Erro ao processar animal ID ${animalId}:`, error.message);
    ...
    console.log(`\nâÅ“¨ Processamento concluÃ­do!`);
    console.log(`âÅ“â€¦ Sucessos: ${sucessos}`);
    console.log(`âÅ’ Erros: ${erros}`);
    
    if (animaisComErro.length > 0) {
      console.log(`\nâÅ¡ ï¸  Animais com erro:`);
