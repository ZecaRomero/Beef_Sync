...
async function checkSaidasSemen() {
  ...
  try {
    console.log('ğÅ¸â€ Verificando saÃ­das de sÃªmen no PostgreSQL...');
    ...
    console.log('\nğÅ¸â€œÅ  Todos os registros na tabela estoque_semen:');
    ...
    console.log(`\nğÅ¸â€œ¤ SaÃ­das encontradas: ${saidas.rows.length}`);
    if (saidas.rows.length > 0) {
      saidas.rows.forEach(saida => {
        console.log(`   ğÅ¸â€¸ ID: ${saida.id}`);
      ...
      console.log('   âÅ’ Nenhuma saÃ­da encontrada!');
    }
    ...
    console.log(`\nğÅ¸â€”â€šï¸  Tabela 'saidas_semen' existe: ${saidasTable.rows[0].exists}`);
    
    if (saidasTable.rows[0].exists) {
      const saidasSeparadas = await client.query('SELECT * FROM saidas_semen ORDER BY created_at DESC');
      console.log(`ğÅ¸â€œâ€¹ Registros na tabela saidas_semen: ${saidasSeparadas.rows.length}`);
      
      if (saidasSeparadas.rows.length > 0) {
        saidasSeparadas.rows.forEach(saida => {
          console.log(`   ğÅ¸â€¸ ID: ${saida.id} | Entrada ID: ${saida.entrada_id} | Destino: ${saida.destino} | Doses: ${saida.quantidade_doses}`);
        });
      }
    }
    ...
    console.log(`\nğÅ¸â€œ¥ Entradas encontradas: ${entradas.rows.length}`);
    entradas.rows.forEach(entrada => {
      console.log(`   ğÅ¸â€¸ ID: ${entrada.id} | Touro: ${entrada.nome_touro} | Total: ${entrada.quantidade_doses} | DisponÃ­vel: ${entrada.doses_disponiveis} | Usadas: ${entrada.doses_usadas} | Status: ${entrada.status}`);
    });
