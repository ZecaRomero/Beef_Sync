...
async function main() {
  console.log('≈∏‚Äùß Ajustando schema de lotes...')
  try {
    const tableStatus = await ensureTableExists()
    const colStatus = await fixNumeroLoteColumn()
    console.log(`≈∏‚Äú¶ coluna numero_lote: ${colStatus}`)

    await recreateGerarProximoLoteFunction()
    console.log('‚≈ì‚Ä¶ Fun√ß√£o gerar_proximo_lote recriada com sucesso')
    ...
    `)
    console.log(`≈∏ß™ Lote gerado: ${sample.rows[0].numero_lote}`)

    console.log('≈∏≈Ω‚Ä∞ Ajuste conclu√≠do')
    process.exit(0)
  } catch (err) {
    console.error('‚ù≈í Erro no ajuste:', err.message)
    process.exit(1)
  }
}
