...
async function verificar() {
  ...
  try {
    ...
    console.log('�œ… NF 2141 encontrada\n')
    ...
    const itens = await query(`
      ...
      ORDER BY id
    `, [nf.rows[0].id])
    console.log(`�Ÿ“‹ Itens na NF (notas_fiscais_itens): ${itens.rows.length}`)
    ...
    const teReceptoras = await query(`
      ...
      ORDER BY a.serie
    `)
    console.log(`\n�Ÿ“‹ Animais com TE da NF 2141 (no banco): ${teReceptoras.rows.length}`)
    ...
    const todasM = await query(`
      ...
      ORDER BY serie
    `)
    console.log(`\n�Ÿ“‹ Total de receptoras (Mestiça) no banco: ${todasM.rows.length}`)
