...
async function testInsert() {
  ...
  try {
    ...
    console.log('≈∏‚Äúù Inserindo animal de teste:', testAnimal);
    ...
    console.log('‚≈ì‚Ä¶ Animal inserido:', insertedAnimal);
    ...
    if (verificar.rows.length > 0) {
      console.log('‚≈ì‚Ä¶ Animal encontrado no banco:', verificar.rows[0]);
    } else {
      console.error('‚ù≈í Animal N√∆íO encontrado ap√≥s INSERT!');
    }
    
    // Contar novamente
    const depois = await client.query('SELECT COUNT(*) as total FROM animais');
    console.log(`≈∏‚Äú≈† Animais depois do teste: ${depois.rows[0].total}`);
    ...
    console.log(`≈∏‚Äú≈† Animais ap√≥s limpeza: ${final.rows[0].total}`);
    
    console.log('‚≈ì‚Ä¶ Teste conclu√≠do com sucesso!');
