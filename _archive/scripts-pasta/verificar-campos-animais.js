...
async function verificar() {
  try {
    initDatabase()
    console.log('�Ÿ”� VERIFICANDO CAMPOS DOS ANIMAIS NO BANCO\n')
    ...
    `)
    console.log('�Ÿ“‹ Colunas existentes na tabela animais:')
    cols.rows.forEach(r => console.log(`   - ${r.column_name} (${r.data_type})`))
    if (cols.rows.length === 0) console.log('   �š�️ Nenhuma das colunas críticas encontrada!')
    ...
    // 4. �šltimos 5 animais atualizados
    ...
    `)
    console.log('\n�Ÿ“… �šltimos 5 animais (por updated_at):')
    ...
    console.error('�Œ Erro:', e.message)
    process.exit(1)
  }
  process.exit(0)
}
