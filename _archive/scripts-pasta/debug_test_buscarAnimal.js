...
async function test() {
    try {
        ...
        if (animal) {
            console.log('‚≈ì‚Ä¶ Found animal:', animal.serie, animal.rg, 'ID:', animal.id);
        } else {
            console.log('‚ù≈í Animal not found with "16207"');
        }
        
        console.log('Testing buscarAnimalPorId with ID 1175...');
        const animalId = await service.buscarAnimalPorId(1175);
        if (animalId) {
             console.log('‚≈ì‚Ä¶ Found animal by ID:', animalId.serie, animalId.rg, 'ID:', animalId.id);
        } else {
             console.log('‚ù≈í Animal not found by ID 1175');
        }

    } catch (e) {
        console.error('‚ù≈í Error:', e);
    }
    process.exit(0);
}
test();
