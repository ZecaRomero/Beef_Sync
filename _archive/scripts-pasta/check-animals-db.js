...
async function checkAnimals() {
  try {
    console.log('≈∏‚Äùç Verificando animais no PostgreSQL...');
    
    // Verificar total de animais
    const result = await query('SELECT COUNT(*) as total FROM animais');
    console.log('≈∏‚Äú≈† Total de animais:', result.rows[0].total);
    ...
    console.log('≈∏ê‚Äû Receptoras encontradas:', receptoras.rows.length);
    ...
    console.log('\n≈∏‚Äú‚Äπ √≈°ltimos 10 animais cadastrados:');
    ...
    console.log('\n≈∏‚Äî‚ÄöÔ∏è Estrutura da tabela animais:');
