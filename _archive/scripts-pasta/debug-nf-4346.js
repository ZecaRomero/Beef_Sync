...
async function debugNF4346() {
  try {
    console.log('≈∏‚Äùç Buscando NF 4346...')
    ...
    if (nfResult.rows.length === 0) {
      console.log('‚ù≈í NF 4346 n√£o encontrada!')
      return
    }
    
    const nf = nfResult.rows[0]
    console.log('‚≈ì‚Ä¶ NF 4346 encontrada:', {
      id: nf.id,
    ...
    console.log(`\n≈∏‚Äú¶ Total de itens encontrados: ${itensResult.rows.length}`)
    ...
    console.log(`\n≈∏‚Äú≈† Total de animais calculado: ${totalAnimais}`)
    ...
    console.error('‚ù≈í Erro ao debugar NF 4346:', error)
  } finally {
    process.exit(0)
  }
}
