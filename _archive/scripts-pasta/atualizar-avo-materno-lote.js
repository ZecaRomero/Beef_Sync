...
async function atualizarAvoMaternoLote(arquivoCSV) {
  ...
  try {
    ...
    if (!fs.existsSync(caminhoArquivo)) {
      console.error(`‚ù≈í Arquivo n√£o encontrado: ${caminhoArquivo}`)
      console.log('\n≈∏‚Äô° Crie um arquivo CSV com o formato:')
    ...
    if (linhas.length < 2) {
      console.error('‚ù≈í Arquivo CSV deve ter pelo menos uma linha de cabe√ßalho e uma linha de dados')
      process.exit(1)
    }
    ...
    console.log(`≈∏‚Äú‚Äπ Processando ${dados.length} animais...\n`)
    ...
    for (let i = 0; i < dados.length; i++) {
      ...
      if (campos.length < 3) {
        console.log(`‚≈°†Ô∏è Linha ${i + 2} inv√°lida (formato: serie,rg,avo_materno): ${linha}`)
      ...
      if (!serie || !rg || !avoMaterno) {
        console.log(`‚≈°†Ô∏è Linha ${i + 2} com campos vazios: ${linha}`)
      ...
      try {
        ...
        if (animalResult.rows.length === 0) {
          console.log(`‚ù≈í Animal ${serie}-${rg} n√£o encontrado`)
        ...
        if (animal.avo_materno === avoMaterno) {
          console.log(`‚‚ÄûπÔ∏è  ${serie}-${rg} j√° tem esse av√¥ materno: "${avoMaterno}"`)
        ...
        console.log(`‚≈ì‚Ä¶ ${serie}-${rg}: "${animal.avo_materno || 'NULL'}" ‚‚Ä†‚Äô "${avoMaterno}"`)
        resultados.sucessos.push({ 
          serie, 
          rg, 
          avoMaterno, 
        ...
        console.error(`‚ù≈í Erro ao processar ${serie}-${rg}:`, error.message)
    ...
    console.log(`\n≈∏‚Äú‚Äû Log salvo em: ${logFile}`)
    
  } catch (error) {
    console.error('‚ù≈í Erro fatal:', error.message)
    throw error
  } finally {
    client.release()
    await pool.end()
...
if (!arquivoCSV) {
  console.log('≈∏‚Äú‚Äπ Script para atualizar av√¥ materno de animais em lote\n')
  console.log('Uso:')
