...
async function corrigir() {
  console.log('≈∏‚Äùß Corrigindo IAs de prenhez (CJCJ 15959 e CJCJ 15668)\n')
  ...
  try {
    ...
    for (const { serie, rg, touroPrenha } of CORRECOES) {
      ...
      if (animal.rows.length === 0) {
        console.log(`   ‚≈°†Ô∏è Animal ${serie} ${rg} n√£o encontrado.`)
        continue
      }
      const animalId = animal.rows[0].id
      const nome = animal.rows[0].nome || `${serie} ${rg}`
      console.log(`\n   ≈∏‚Äú≈í ${serie} ${rg} (${nome}) - ID ${animalId}`)
    ...
    console.log(`\n   ≈∏‚Äú‚Äπ Total: ${totalAtualizados} registro(s) atualizado(s).`)
    console.log('\n   ≈∏‚Äô° Recarregue as p√°ginas de consulta para ver os touros corretos.')
  } catch (err) {
