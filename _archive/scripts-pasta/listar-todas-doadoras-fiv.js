...
async function listarTodasDoadorasFIV() {
  try {
    console.log('≈∏‚Äùç Listando todas as doadoras com coletas FIV...\n')
    ...
    console.log(`≈∏‚Äú≈† Total de coletas FIV: ${coletas.rows.length}\n`)
    ...
    console.log(`≈∏‚Äú‚Äπ Total de doadoras √∫nicas: ${Object.keys(coletasPorDoadora).length}\n`)
    console.log('‚‚Äù‚Ç¨'.repeat(120))
    console.log('≈∏‚Äú≈† RESUMO POR DOADORA:\n')
    ...
    Object.keys(coletasPorDoadora).sort().forEach((key, index) => {
      ...
      doadora.coletas.forEach((coleta, idx) => {
        ...
        if (ano < 2020 || ano > 2030) {
          suspeita = ' ‚≈°†Ô∏è DATA SUSPEITA'
        } else if (ano === 2027 || ano === 2028 || ano === 2029) {
          suspeita = ' ‚≈°†Ô∏è DATA FUTURA'
        }
    ...
    if (doadorasComDatasSuspeitas.length > 0) {
      console.log(`\n‚≈°†Ô∏è  ${doadorasComDatasSuspeitas.length} doadoras com datas suspeitas (fora do intervalo 2020-2030):`)
      doadorasComDatasSuspeitas.forEach(key => {
        console.log(`   - ${key}`)
      })
    } else {
      console.log('\n‚≈ì‚Ä¶ Nenhuma doadora com datas fora do intervalo razo√°vel (2020-2030)')
    }
