...
const texto = `S√‚Ä∞RIE	RG	LOCAL	ACASALAMENTO	DATA IA	DATA DG	RESULT
...
const textoSpaces = `S√‚Ä∞RIE     RG     LOCAL     ACASALAMENTO     DATA IA     DATA DG     RESULT
123       456    Piquete 1  TOURO TESTE      01/01/2024  01/02/2024  P
789       012    Piquete 2                   01/01/2024  01/02/2024  N
345       678    Piquete 3  OUTRO TOURO      01/01/2024  01/02/2024  P`;
...
function processText(texto, label) {
    ...
    const temCabecalho = primeiraLinha.includes('S√‚Ä∞RIE') || primeiraLinha.includes('SERIE') || primeiraLinha.includes('LOCAL') || primeiraLinha.includes('ACASALAMENTO') || primeiraLinha.includes('TOURO');
    ...
    if (temCabecalho) {
      ...
      if (colsNaoVazias.length >= 2) {
        ...
        cols.forEach((col, idx) => {
          ...
          if (c.includes('S√‚Ä∞RIE') || c.includes('SERIE')) mapaColunas.serie = idx;
        ...
        console.log('≈∏‚Äî∫Ô∏è Mapa de colunas detectado:', mapaColunas);
      }
    }
    ...
    for (let i = 0; i < dadosLinhas.length; i++) {
      ...
      if (!touroIA) console.log('  ‚≈°†Ô∏è FALHA: Touro n√£o encontrado!');
    }
}
