...
async function sincronizarDados(arquivoJson) {
  console.log('≈∏‚Äù‚Äû SINCRONIZANDO DADOS DO LOCALSTORAGE COM POSTGRESQL\n');
  console.log('='.repeat(60));

  try {
    // 1. Ler arquivo JSON
    console.log('\n≈∏‚Äú‚Äö 1. Lendo arquivo JSON...');
    ...
    console.log(`‚≈ì‚Ä¶ Arquivo lido: ${arquivoJson}`);
    console.log(`≈∏‚Äú‚Ä¶ Data do backup: ${dados.timestamp}`);
    ...
    if (dados.animals && dados.animals.length > 0) {
      console.log(`\n≈∏ê‚Äû 2. Sincronizando ${dados.animals.length} animais...`);
      ...
      for (const animal of dados.animals) {
        try {
          ...
          if (existe.rows.length > 0) {
            ...
            console.log(`   ‚≈ì‚Ä¶ Atualizado: ${animal.serie}-${animal.rg}`);
          } else {
            // Inserir novo animal
            await pool.query(`
              INSERT INTO animais (
            ...
            stats.animais.novos++;
            console.log(`   ‚≈ì‚Ä¶ Novo: ${animal.serie}-${animal.rg}`);
          }
        } catch (error) {
          stats.animais.erros++;
          console.error(`   ‚ù≈í Erro ao sincronizar ${animal.serie}-${animal.rg}:`, error.message);
        }
      }
    }
    ...
    if (dados.deaths && dados.deaths.length > 0) {
      console.log(`\n≈∏‚Äô‚Ç¨ 3. Sincronizando ${dados.deaths.length} mortes...`);
      ...
      for (const morte of dados.deaths) {
        try {
          ...
          if (animal.rows.length === 0) {
            console.warn(`   ‚≈°†Ô∏è Animal n√£o encontrado: ${morte.serie}-${morte.rg}`);
            stats.mortes.erros++;
            continue;
          }
          ...
          if (morteExiste.rows.length === 0) {
            ...
            stats.mortes.novos++;
            console.log(`   ‚≈ì‚Ä¶ Morte registrada: ${morte.serie}-${morte.rg}`);
          } else {
            console.log(`   ‚≈°†Ô∏è Morte j√° existe: ${morte.serie}-${morte.rg}`);
          }
        } catch (error) {
          stats.mortes.erros++;
          console.error(`   ‚ù≈í Erro ao sincronizar morte:`, error.message);
        }
      }
    }
    ...
    if (dados.dna && dados.dna.length > 0) {
      console.log(`\n≈∏ß¨ 4. Sincronizando ${dados.dna.length} registros de DNA...`);
      ...
      for (const dna of dados.dna) {
        try {
          ...
          if (existe.rows.length === 0) {
            ...
            stats.dna.novos++;
            console.log(`   ‚≈ì‚Ä¶ DNA registrado: ${dna.serie}-${dna.rg}`);
          }
        } catch (error) {
          stats.dna.erros++;
          console.error(`   ‚ù≈í Erro ao sincronizar DNA:`, error.message);
        }
      }
    }
    ...
    if (dados.nitrogenio && dados.nitrogenio.length > 0) {
      console.log(`\n‚ù‚ÄûÔ∏è 5. Sincronizando ${dados.nitrogenio.length} registros de nitrog√™nio...`);
      ...
      for (const n of dados.nitrogenio) {
        try {
          ...
          console.log(`   ‚≈ì‚Ä¶ Nitrog√™nio registrado: ${n.data}`);
        } catch (error) {
          stats.nitrogenio.erros++;
          console.error(`   ‚ù≈í Erro ao sincronizar nitrog√™nio:`, error.message);
        }
      }
    }
    ...
    if (dados.exames && dados.exames.length > 0) {
      console.log(`\n≈∏‚Äù¨ 6. Sincronizando ${dados.exames.length} exames androl√≥gicos...`);
      ...
      for (const exame of dados.exames) {
        try {
          ...
          stats.exames.novos++;
          console.log(`   ‚≈ì‚Ä¶ Exame registrado: ${exame.touro}`);
        } catch (error) {
          stats.exames.erros++;
          console.error(`   ‚ù≈í Erro ao sincronizar exame:`, error.message);
        }
      }
    }
    ...
    if (dados.custos && dados.custos.length > 0) {
      console.log(`\n≈∏‚Äô∞ 7. Sincronizando ${dados.custos.length} custos...`);
      ...
      for (const custo of dados.custos) {
        try {
          ...
          if (animal.rows.length === 0) {
            console.warn(`   ‚≈°†Ô∏è Animal n√£o encontrado: ${custo.serie}-${custo.rg}`);
            stats.custos.erros++;
            continue;
          }
          ...
          stats.custos.novos++;
          console.log(`   ‚≈ì‚Ä¶ Custo registrado: ${custo.tipo}`);
        } catch (error) {
          stats.custos.erros++;
          console.error(`   ‚ù≈í Erro ao sincronizar custo:`, error.message);
        }
      }
    }
    ...
    // Resumo final
    console.log('\n' + '='.repeat(60));
    console.log('‚≈ì‚Ä¶ SINCRONIZA√‚Ä°√∆íO CONCLU√çDA!\n');
    ...
    console.error('\n‚ù≈í Erro durante sincroniza√ß√£o:', error);
...
if (args.length === 0) {
  console.log('‚ù≈í Uso: node sincronizar-localStorage.js <arquivo.json>');
  console.log('\nExemplo:');
