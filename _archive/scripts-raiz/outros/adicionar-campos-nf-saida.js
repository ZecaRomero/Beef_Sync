...
async function adicionarCamposNFSaida() {
  ...
  try {
    console.log('≈∏‚Äùß Adicionando campos data_saida e motorista na tabela notas_fiscais...');
    ...
    console.log('‚≈ì‚Ä¶ Campos adicionados com sucesso!');
    console.log('');
    ...
    console.error('‚ù≈í Erro ao adicionar campos:', error);
    throw error;
  } finally {
    client.release();
    await pool.end();
...
adicionarCamposNFSaida()
  .then(() => {
    console.log('');
    console.log('‚≈ì‚Ä¶ Script executado com sucesso!');
    process.exit(0);
  })
  .catch((error) => {
