...
async function registrarPrenhasNascimentos() {
  try {
    console.log('ğÅ¸¤° Registrando 46 receptoras prenhas em Nascimentos...\n');
    ...
    console.log(`ğÅ¸â€œÅ  Total de receptoras com DG positivo: ${receptorasPrenhas.length}\n`);
    
    if (receptorasPrenhas.length === 0) {
      console.log('âÅ¡ ï¸ Nenhuma receptora com DG positivo encontrada!');
      return;
    }
    ...
    console.log(`ğÅ¸â€œâ€ Total de NFs de receptoras: ${nfsReceptoras.length}\n`);
    ...
    console.log(`ğÅ¸â€™â€° Total de TEs registradas: ${tes.length}\n`);
    ...
    for (const receptora of receptorasPrenhas) {
      ...
      try {
        ...
        if (!dataTE && receptora.data_chegada) {
          ...
          console.log(`ââ€¹ï¸ ${rgCompleto} - Usando data_chegada como fallback`);
        }
        
        if (!dataTE) {
          console.log(`âÅ¡ ï¸ ${rgCompleto} - Sem data de TE ou data_chegada, pulando...`);
          erros++;
        ...
        if (response.ok) {
          console.log(`âÅ“â€¦ ${rgCompleto} - Registrada em Nascimentos (Parto previsto: ${prevPartoStr})`);
          sucessos++;
          detalhes.push({ rg: rgCompleto, sucesso: true, partoPrevisto: prevPartoStr });
        } else {
          const error = await response.text();
          console.log(`âÅ’ ${rgCompleto} - Erro ao registrar: ${error}`);
          erros++;
          detalhes.push({ rg: rgCompleto, erro: error });
        }
        
      } catch (error) {
        console.log(`âÅ’ ${rgCompleto} - Erro: ${error.message}`);
        erros++;
        detalhes.push({ rg: rgCompleto, erro: error.message });
      }
    }
    
    console.log(`\nğÅ¸â€œË† Resumo Final:`);
    console.log(`   âÅ“â€¦ Registradas com sucesso: ${sucessos}`);
    console.log(`   âÅ’ Erros: ${erros}`);
    console.log(`\nğÅ¸â€™¡ Agora verifique o menu ReproduÃ§Ã£o > Nascimentos para ver as 46 receptoras prenhas!`);
    
  } catch (error) {
    console.error('âÅ’ Erro:', error.message);
    console.log('\nğÅ¸â€™¡ Certifique-se de que o servidor estÃ¡ rodando em http://localhost:3020');
  }
}
