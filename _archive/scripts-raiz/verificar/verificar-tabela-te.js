...
async function verificarTabelaTE() {
  ...
  try {
    ...
    console.log('Tabelas relacionadas a TE/Embri√µes:');
    if (tabelas.rows.length > 0) {
      tabelas.rows.forEach(row => {
        console.log(`- ${row.table_name}`);
      });
    } else {
      console.log('‚ù≈í Nenhuma tabela encontrada');
    }
    ...
    if (colunas.rows.length > 0) {
      ...
      for (const rg of receptoras) {
        ...
        if (result.rows.length > 0) {
          ...
          console.log(`\n‚ù≈í M ${rg}: Nenhuma TE encontrada`);
        }
      }
      ...
      console.log('‚ù≈í Tabela transferencias_embrioes n√£o existe ou est√° vazia');
    }
