...
async function verificarItensNFMarcelo() {
  ...
  try {
    console.log('≈∏‚Äùç VERIFICANDO ITENS DAS NFs DO MARCELO\n');
    ...
    console.log(`≈∏‚Äú‚Äπ NFs do Marcelo: ${nfsResult.rows.length}\n`);
    
    let totalItens = 0;
    
    for (const nf of nfsResult.rows) {
      console.log(`\n≈∏‚Äú¶ NF ${nf.numero_nf} (ID: ${nf.id}):`);
        ...
        itensResult.rows.forEach((item, idx) => {
          ...
          if (item.dados_item) {
            try {
              ...
              console.log(`      ‚≈°†Ô∏è Erro ao parsear dados_item: ${e.message}`);
            }
          }
        });
      } else {
        console.log(`   ‚ù≈í Nenhum item cadastrado nesta NF`);
      }
    }
    ...
    console.log('\n\n≈∏‚Äùç VERIFICANDO ESTRUTURA DA TABELA notas_fiscais_itens:');
    ...
    if (estruturaResult.rows.length > 0) {
      console.log('\n‚≈ì‚Ä¶ Tabela existe com as seguintes colunas:');
      estruturaResult.rows.forEach(col => {
        console.log(`   - ${col.column_name} (${col.data_type})`);
      });
    } else {
      console.log('\n‚ù≈í Tabela notas_fiscais_itens N√∆íO existe!');
    }
