...
async function verificarDataTE() {
  try {
    console.log('�Ÿ”� Verificando data_te das receptoras prenhas...\n');
    ...
    console.log(`�Ÿ“„ Total de NFs: ${nfs.length}\n`);
    
    // 3. Verificar data_te para cada receptora
    let comDataTE = 0;
    let semDataTE = 0;
    
    console.log('�Ÿ“‹ Análise detalhada:\n');
    ...
    for (const receptora of receptorasPrenhas) {
      ...
      if (nfReceptora) {
        if (nfReceptora.data_te) {
          console.log(`�œ… ${rgCompleto} - NF ${nfReceptora.numero_nf} - Data TE: ${new Date(nfReceptora.data_te).toLocaleDateString('pt-BR')}`);
          comDataTE++;
        } else {
          console.log(`�š�️ ${rgCompleto} - NF ${nfReceptora.numero_nf} - SEM data_te`);
          semDataTE++;
        }
      } else {
        console.log(`�Œ ${rgCompleto} - NF não encontrada`);
        semDataTE++;
      }
    }
    
    console.log(`\n�Ÿ“ˆ Resumo:`);
    console.log(`   �œ… Com data_te: ${comDataTE}`);
    console.log(`   �Œ Sem data_te: ${semDataTE}`);
