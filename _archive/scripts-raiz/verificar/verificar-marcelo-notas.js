...
async function verificarMarceloNotas() {
  ...
  try {
    console.log('≈∏‚Äùç INVESTIGANDO NOTAS DO MARCELO\n');
    console.log('='.repeat(80));
    
    // 1. Buscar todas as notas fiscais do Marcelo
    console.log('\n≈∏‚Äú‚Äπ 1. NOTAS FISCAIS DO MARCELO:');
    ...
    for (const nf of notasResult.rows) {
      ...
      if (itensResult.rows.length === 0) {
        console.log(`   ‚ù≈í Nenhum item cadastrado`);
      } else {
        console.log(`   ‚≈ì‚Ä¶ ${itensResult.rows.length} itens cadastrados:`);
    ...
    // 3. Buscar animais vinculados √†s notas
    console.log('\n\n≈∏êÆ 3. ANIMAIS VINCULADOS √‚Ç¨S NOTAS:');
    for (const nf of notasResult.rows) {
      ...
      if (animaisResult.rows.length === 0) {
        console.log(`   ‚ù≈í Nenhum animal vinculado`);
      } else {
        console.log(`   ‚≈ì‚Ä¶ ${animaisResult.rows.length} animais vinculados:`);
        ...
        console.log(`\n      ≈∏‚Äúù Primeiros 5 animais:`);
        animaisResult.rows.slice(0, 5).forEach((animal, idx) => {
          console.log(`         ${idx + 1}. RG: ${animal.rg} | Nome: ${animal.nome || 'S/N'} | ${animal.sexo} | ${animal.categoria} | ${animal.situacao} | ${animal.inativo ? '≈∏‚Äù¥ INATIVO' : '≈∏≈∏¢ ATIVO'}`);
        });
      }
    }
    
    // 4. Buscar todos os animais do Marcelo (independente de NF)
    console.log('\n\n≈∏‚Äùç 4. BUSCA GERAL DE ANIMAIS DO MARCELO:');
    ...
    // 5. Verificar se h√° animais sem NF mas com fornecedor Marcelo
    console.log('\n\n≈∏‚Äùç 5. ANIMAIS SEM NF MAS COM FORNECEDOR MARCELO:');
    ...
    if (semNFResult.rows.length > 0) {
      console.log(`\n‚≈°†Ô∏è Encontrados ${semNFResult.rows.length} animais sem NF:`);
      semNFResult.rows.slice(0, 10).forEach((animal, idx) => {
        console.log(`   ${idx + 1}. RG: ${animal.rg} | Fornecedor: ${animal.fornecedor} | ${animal.categoria} | ${animal.inativo ? '≈∏‚Äù¥ INATIVO' : '≈∏≈∏¢ ATIVO'}`);
      });
    } else {
      console.log(`\n‚≈ì‚Ä¶ Todos os animais do Marcelo t√™m NF vinculada`);
    }
    
    console.log('\n' + '='.repeat(80));
    console.log('\n‚≈ì‚Ä¶ An√°lise conclu√≠da!');
