...
async function verificarNFQuantidade() {
  console.log('ðÅ¸â€ VERIFICANDO QUANTIDADES NAS NOTAS FISCAIS\n')
  ...
  try {
    ...
    console.log(`\nðÅ¸â€œâ€¹ ÃÅ¡LTIMAS ${nfs.rows.length} NOTAS FISCAIS:\n`)
    console.log('-'.repeat(80))
    
    for (const nf of nfs.rows) {
      console.log(`\nðÅ¸â€œâ€ž NF: ${nf.numero_nf} (${nf.tipo})`)
      ...
      if (itens.rows.length > 0) {
        console.log(`   ðÅ¸â€œ¦ ${itens.rows.length} itens encontrados:`)
        ...
        itens.rows.forEach((row, idx) => {
          try {
            ...
            console.error(`      âÅ’ Erro ao processar item ${idx + 1}:`, e.message)
          }
        })
        ...
        if (totalAnimais === 0) {
          console.log(`      âÅ¡ ï¸  PROBLEMA: Quantidade total Ã© ZERO!`)
        }
      } else {
        console.log(`   âÅ¡ ï¸  Nenhum item encontrado na tabela notas_fiscais_itens`)
        ...
        if (nfCompleta.rows.length > 0 && nfCompleta.rows[0].itens) {
          try {
            ...
            console.log(`   ðÅ¸â€œ¦ ${itensArray.length} itens encontrados no campo JSONB`)
            ...
            console.error(`   âÅ’ Erro ao parsear itens JSONB:`, e.message)
          }
        }
      }
    }
