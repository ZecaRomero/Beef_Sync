...
console.log('≈∏‚Äùç Verificando backups JSON para dados de DNA, Nitrog√™nio e Androl√≥gicos...\n');
    ...
    if (data.data) {
      for (const tabela of tabelasProcuradas) {
        if (data.data[tabela] && data.data[tabela].length > 0) {
          ...
          console.log(`   ‚≈ì‚Ä¶ ${tabela}: ${data.data[tabela].length} registros`);
        }
      }
    }
    
  } catch (err) {
    console.log(`   ‚≈°†Ô∏è  ${file}: Erro ao ler - ${err.message}`);
  }
}
