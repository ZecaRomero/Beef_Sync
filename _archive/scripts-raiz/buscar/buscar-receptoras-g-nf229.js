...
async function buscarReceptorasG() {
  ...
  try {
    console.log('≈∏‚Äùç BUSCANDO RECEPTORAS G DA NF 229\n');
    console.log('='.repeat(80));
    
    // 1. Buscar na tabela animais
    console.log('\n≈∏êÆ 1. BUSCA NA TABELA ANIMAIS:');
    ...
    if (animaisResult.rows.length > 0) {
      ...
      console.log('‚ù≈í Nenhum animal encontrado com essas tatuagens');
    }
    
    // 2. Buscar todas as receptoras G (para ver o padr√£o)
    console.log('\n≈∏‚Äùç 2. TODAS AS RECEPTORAS COM S√‚Ä∞RIE G:');
    ...
    console.log('\n\n≈∏‚Äú¶ 3. BUSCA NA TABELA notas_fiscais_itens:');
    ...
    if (nfResult.rows.length > 0) {
      ...
      if (itensResult.rows.length > 0) {
        console.log('‚≈ì‚Ä¶ Itens encontrados:');
        itensResult.rows.forEach((item, idx) => {
          ...
          if (item.dados_item) {
            try {
              ...
              console.log(`   ‚≈°†Ô∏è Erro ao parsear dados_item`);
            }
          }
        });
      } else {
        console.log('‚ù≈í Nenhum item cadastrado na NF 229');
      }
    } else {
      console.log('‚ù≈í NF 229 n√£o encontrada');
    }
    
    // 4. Buscar itens com tatuagens G espec√≠ficas em TODAS as NFs
    console.log('\n\n≈∏‚Äùç 4. BUSCAR TATUAGENS G 355, G 338, G 354 EM TODAS AS NFs:');
    ...
    if (todasNFsResult.rows.length > 0) {
      ...
      console.log('‚ù≈í Nenhuma tatuagem G 355, G 338 ou G 354 encontrada em nenhuma NF');
    }
    
    // 5. Buscar receptoras G com n√∫meros pr√≥ximos (350-360)
    console.log('\n\n≈∏‚Äùç 5. RECEPTORAS G COM N√≈°MEROS ENTRE 350-360:');
