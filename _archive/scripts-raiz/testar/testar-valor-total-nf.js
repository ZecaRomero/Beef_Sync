...
async function testarValorTotalNF() {
  ...
  try {
    console.log('≈∏‚Äùç Verificando valores totais das notas fiscais...\n');
    ...
    console.log(`≈∏‚Äú≈† √≈°ltimas ${result.rows.length} Notas Fiscais:\n`);
    ...
    result.rows.forEach(nf => {
      ...
      const status = diferenca < 0.01 ? '‚≈ì‚Ä¶' : '‚≈°†Ô∏è';
      ...
      if (diferenca >= 0.01) {
        console.log(`   ‚≈°†Ô∏è Diferen√ßa: R$ ${diferenca.toFixed(2)}`);
      }
      console.log('');
    });
