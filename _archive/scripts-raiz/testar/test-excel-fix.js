...
async function testExcelGeneration() {
  console.log('ðÅ¸§ª Testando geraÃ§Ã£o de Excel corrigida...')
  ...
  try {
    ...
    console.log('ðÅ¸â€œÅ  Gerando relatÃ³rio Excel...')
    const buffer = await generateExcelReport(reportData, period)
    
    console.log(`âÅ“â€¦ Excel gerado com sucesso! Tamanho: ${buffer.length} bytes`)
    
    // Salvar arquivo para teste
    fs.writeFileSync('teste-excel-corrigido.xlsx', buffer)
    console.log('ðÅ¸â€™¾ Arquivo salvo como: teste-excel-corrigido.xlsx')
    
    return true
    
  } catch (error) {
    console.error('âÅ’ Erro ao gerar Excel:', error.message)
...
testExcelGeneration().then(success => {
  if (success) {
    console.log('\nðÅ¸Å½â€° Teste concluÃ­do com sucesso!')
  } else {
    console.log('\nðÅ¸â€™¥ Teste falhou!')
  }
