...
async function testarDadosPDF() {
  try {
    console.log('≈∏‚Äùç Testando busca de dados para PDF...\n')
    ...
    if (enviosResult.rows.length === 0) {
      console.log('‚ù≈í Nenhum envio de DNA encontrado')
      return
    }
    
    const envio = enviosResult.rows[0]
    console.log('≈∏‚Äú¶ Envio encontrado:', {
      id: envio.id,
    ...
    console.log(`\n≈∏ê‚Äû ${animaisResult.rows.length} animal(is) vinculado(s) ao envio\n`)
    ...
    for (const row of animaisResult.rows.slice(0, 3)) { // Apenas 3 primeiros para teste
      ...
      if (animalResult.rows.length === 0) {
        console.log(`‚ù≈í Animal ${animalId} n√£o encontrado`)
        continue
      }
      
      const animal = animalResult.rows[0]
      console.log(`\n≈∏‚Äú‚Äπ Animal ${animal.serie}-${animal.rg}:`)
      ...
      if (animal.pai_id) {
        ...
        if (paiResult.rows.length > 0) {
          ...
          console.log(`  ‚≈°†Ô∏è Pai ID ${animal.pai_id} n√£o encontrado no banco`)
        }
      }
      ...
      if (animal.mae_id) {
        ...
        if (maeResult.rows.length > 0) {
          ...
          console.log(`  ‚≈°†Ô∏è M√£e ID ${animal.mae_id} n√£o encontrada no banco`)
        }
      }
    }
