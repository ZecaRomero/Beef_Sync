  ...
  try {
    ...
    for (const receptora of receptoras) {
      ...
      if (result.rows.length > 0) {
        console.log(`‚≈ì‚Ä¶ M ${receptora.rg} (TE ID: ${receptora.te_id})`);
        console.log(`   Nova data TE: ${new Date(result.rows[0].data_te).toLocaleDateString('pt-BR')}`);
        corrigidas++;
      } else {
        console.log(`‚ù≈í M ${receptora.rg}: Erro ao atualizar`);
      }
    }
    
    console.log(`\n≈∏‚Äú≈† RESUMO:`);
    console.log(`‚≈ì‚Ä¶ ${corrigidas} de ${receptoras.length} receptoras corrigidas`);
    
    // Verificar se as datas foram atualizadas
    console.log('\n=== VERIFICANDO ATUALIZA√‚Ä°√‚Ä¢ES ===\n');
    ...
    for (const receptora of receptoras) {
      ...
      if (result.rows.length > 0) {
        ...
        const status = dataAtual === '27/11/2025' ? '‚≈ì‚Ä¶' : '‚ù≈í';
