...
async function createTestData() {
  try {
    console.log('≈∏‚Äùç Criando dados de teste...');
    ...
    console.log('≈∏ê‚Äû Criando animais...');
    for (const animal of animals) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`‚≈ì‚Ä¶ Animal ${animal.serie}-${animal.rg} criado com sucesso`);
        } else {
          const error = await response.text();
          console.log(`‚ù≈í Erro ao criar animal ${animal.serie}-${animal.rg}:`, error);
        }
      } catch (error) {
        console.log(`‚ù≈í Erro ao criar animal ${animal.serie}-${animal.rg}:`, error.message);
      }
    }

    // Criar alguns nascimentos
    console.log('\n≈∏‚Äò∂ Criando nascimentos...');
    const births = [
      {
    ...
    for (const birth of births) {
      try {
        ...
        if (response.ok) {
          console.log(`‚≈ì‚Ä¶ Nascimento de ${birth.sexo} criado com sucesso`);
        } else {
          const error = await response.text();
    ...
    if (reportResponse.ok) {
      ...
      console.log('≈∏‚Äú≈† Preview do relat√≥rio com dados:', JSON.stringify(reportData, null, 2));
    }
