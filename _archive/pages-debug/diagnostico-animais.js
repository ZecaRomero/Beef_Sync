  ...
  const carregarDiagnostico = async () => {
    try {
      ...
      if (diag.localStorage > 0 && diag.api === 0) {
        diag.problemas.push('‚ù≈í API n√£o est√° retornando animais (poss√≠vel problema no PostgreSQL)')
      }
      
      if (diag.localStorage === 0) {
        diag.problemas.push('‚≈°†Ô∏è Nenhum animal no localStorage')
      }
      
      if (diag.diferenca > 0) {
        diag.problemas.push(`‚≈°†Ô∏è ${diag.diferenca} animal(is) no localStorage n√£o aparecem na API`)
      }
  ...
  const sincronizarDados = () => {
    ...
    localStorageAnimals.forEach(async (animal) => {
      try {
        ...
        if (response.ok) {
          console.log('‚≈ì‚Ä¶ Animal sincronizado:', animal.serie, animal.rg)
        } else {
          console.error('‚ù≈í Erro ao sincronizar:', animal.serie, animal.rg)
        }
      } catch (error) {
        console.error('‚ù≈í Erro na sincroniza√ß√£o:', error)
      }
    })
    
    alert('≈∏‚Äù‚Äû Sincroniza√ß√£o iniciada! Verifique o console para detalhes.')
      ...
      <SimpleCard>
        <CardHeader>
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            ≈∏‚Äú‚Äπ Instru√ß√µes para Resolver Problemas
          </h3>
        </CardHeader>
        <CardBody>
          <div className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <p>‚‚Ç¨¢ Verifique se o PostgreSQL est√° rodando</p>
              <p>‚‚Ç¨¢ Use o bot√£o "Sincronizar com API" acima</p>
              <p>‚‚Ç¨¢ Verifique os logs do servidor</p>
            </div>
            
            <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
              <p><strong>2. Para cadastrar novos animais:</strong></p>
              <p>‚‚Ç¨¢ V√° em "Animais" ‚‚Ä†‚Äô "Lista de Animais"</p>
              <p>‚‚Ç¨¢ Clique em "+ Novo Animal"</p>
              <p>‚‚Ç¨¢ Preencha os dados obrigat√≥rios</p>
            </div>
            
            <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
              <p><strong>3. Se o problema persistir:</strong></p>
              <p>‚‚Ç¨¢ Reinicie o servidor (Ctrl+C e npm run dev)</p>
              <p>‚‚Ç¨¢ Verifique a conex√£o com o banco de dados</p>
              <p>‚‚Ç¨¢ Limpe o cache do navegador</p>
            </div>
          </div>
        </CardBody>
      </SimpleCard>
